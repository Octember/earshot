...
class CapturingAdapter implements SurfaceAdapter {
  ...
  async updateMessage(_v: string, id: string, text: string) {
    this.events.push(`EDIT #${id} → ${JSON.stringify(text)}`);
  }
  async addReaction() {}
  ...
  async appendStream(_v: string, id: string, delta: string) {
    this.events.push(`APPEND ${id} → ${JSON.stringify(delta)}`);
  }
  async stopStream(_v: string, id: string) {
...
adapter.emit({
  ...
  isBot: false,
  text: `<@${botUserId}> what's 2+2? one word.`,
  ts: "1.0",
  threadRootTs: "root-1",
  mentionsBotId: true,
  deliveryId: "d1",

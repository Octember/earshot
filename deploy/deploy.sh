...
REF=${2:-origin/main}
git fetch -q origin
git archive "$REF" src package.json bun.lock tsconfig.json deploy/policy.yaml | ssh "$HOST" '
  export XDG_RUNTIME_DIR=/run/user/$(id -u)
  cd ~/earshot
  rm -rf src.new && mkdir src.new && tar -x -C src.new
  rm -rf src && mv src.new/src src && mv src.new/package.json package.json && mv src.new/bun.lock bun.lock && mv src.new/tsconfig.json tsconfig.json

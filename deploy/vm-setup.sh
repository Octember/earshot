#!/usr/bin/env bash
# earshot VM setup — runs once on first boot of an exe.dev VM (passed to `exe.dev new --setup-script`).
# Makes a fresh VM ready to run the earshot daemon: keyless codex via the exe-llm gateway + bun. The
# earshot code + secrets are NOT baked in here — they're rsync'd over after boot (earshot is a private
...
#   ssh exe.dev integrations attach llm vm:earshot-daemon      # keyless codex via the exe-llm gateway
set -e
...
# 2. bun (the base image ships codex but not bun; earshot is bun-based).
[ -x "$HOME/.bun/bin/bun" ] || curl -fsSL https://bun.sh/install | bash

# 3. PATH for codex's `bash -lc` subprocesses and for running earshot. codex sources ~/.bash_profile;

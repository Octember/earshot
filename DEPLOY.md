...
`tag` is one Bun process, one SQLite file, zero external services. Deploying it is: install Bun,
point it at a policy file + Slack secrets, and run `tag start` under a supervisor that restarts it
...
routes through. `tag` runs wherever you put it (your Mac, a Linux VM) and drives Codex through the
...
- **Codex CLI**, logged in: `codex login status` should print `Logged in ...`. This is what bills
  to the ChatGPT/Codex plan; tag never calls the Claude/Anthropic API.
...
3. **Event Subscriptions** → enable → **Subscribe to bot events**: `message.channels`,
   `message.groups`, `message.im`, `message.mpim`. **Do not** add `app_mention` — tag detects
...
are `$VAR` indirections only (validated present, never printed). Run `tag doctor` to confirm it
...
| var                | default          | meaning                          |
|--------------------|------------------|----------------------------------|
| `TAG_DB`           | `./tag.db`       | the SQLite ledger file           |
| `TAG_POLICY`       | `./policy.yaml`  | the policy file                  |
| `TAG_STATUS_PORT`  | (off)            | if set, serve a read-only JSON status endpoint on this port |
...
```sh
tag start          # or: bun run src/main.ts start
```

**As a supervised service:**

- **macOS (launchd):** edit the paths in `deploy/com.tag.daemon.plist`, then
  ```sh
  cp deploy/com.tag.daemon.plist ~/Library/LaunchAgents/
  launchctl load ~/Library/LaunchAgents/com.tag.daemon.plist
  ```
  Stop/reload: `launchctl unload ...` / `launchctl load ...` (sends SIGTERM → graceful drain).

- **Linux (systemd):** edit paths/User in `deploy/tag.service`, then
  ```sh
  sudo cp deploy/tag.service /etc/systemd/system/
  sudo systemctl daemon-reload && sudo systemctl enable --now tag
  ```
  `systemctl restart tag` / `stop tag` send SIGTERM; `TimeoutStopSec=180` lets in-flight work drain
  before SIGKILL.
...
- **Status:** `tag status` (human) or `tag status --json` (machine). Shows per-identity
  open/running/waiting/parked counts, spend this month, and timers due/pending. With
  `TAG_STATUS_PORT` set, the same snapshot is served at `http://localhost:$PORT/`.
- **Logs:** structured JSON lines on stdout — the supervisor captures them (launchd →
  `StandardOutPath`; systemd → `journalctl -u tag`). Each line carries `identity_id` and, where
...
- **Live policy reload (§16.2):** edit `policy.yaml` while running — tag watches the file and
...
The entire durability layer is one file (`$TAG_DB`). Back it up WAL-safely with SQLite's online
backup (safe while running):

```sh
sqlite3 "$TAG_DB" ".backup '/backups/tag-$(date +%F).db'"
```

A daily cron is plenty for a homebrew deploy. **Restore** = stop the service, copy a backup over
`$TAG_DB`, start again — restart recovery self-heals any `active` tasks in the restored file.

---

## 7. Rollback

`tag` is a git checkout. To roll back a bad deploy: `git checkout <good-sha>`, restart the service.
...
6. `tag start` → look for `service started` in the logs
7. mention the bot in a channel → it acks/replies; `tag status` shows the activity

...
import { getTask, requireTask, transition, type Post, type Task, type WaitingOn } from "./tasks";
...
export interface FireDueTimersOpts {
  ...
  // SPEC §6.1/§11: posts generated by ledger-originated transitions (the nudge text) are exempt
  // from turn posting scope — the caller delivers them to the surface. Omitting this drops them,
  // which is a silent no-dangling-threads violation; every real caller must wire it.
...
function applyNudge(db: Database, clock: Clock, timer: TimerRow, opts: FireDueTimersOpts): boolean {
  ...
  const result = transition(db, clock, task.id, "waiting", { type: "nudge_sent", parkDeadline, text });
  for (const post of result.posts) opts.onPost?.(post); // §6.1: "One nudge MUST be posted"
  return true;
}
...
export interface RestartRecoveryResult {
  reopened: string[];
  parked: string[];
  // Crash-loop park reports (SPEC §6.1: parking past the bound must be visible, not silent) —
  // the caller delivers these to the surface.
  posts: Post[];
}
...
// The caller MUST deliver the returned posts (the crash-loop park report) — dropping them ends a
// task silently.
export function interruptOrPark(
...
): { outcome: "reopened" | "parked"; posts: Post[] } {
  ...
  if (nextCount > maxConsecutiveInterruptions) {
    ...
    const result = transition(db, clock, taskId, "parked", {
    ...
    return { outcome: "parked", posts: result.posts };
  }
  transition(db, clock, taskId, "open", { type: "interrupted" });
  return { outcome: "reopened", posts: [] };
}
...
export function recoverFromRestart(
  ...
  for (const { id, consecutive_interruptions } of orphaned) {
    ...
    (result.outcome === "parked" ? parked : reopened).push(id);
    posts.push(...result.posts);
  }

  return { reopened, parked, posts };
}

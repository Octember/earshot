...
function sumSpendThisMonth(
  ...
  const rows = identityId
    ...
    : orm(db)
        .select({ spendAmount: turns.spendAmount, startedAt: turns.startedAt })
        .from(turns)
        .where(gte(turns.startedAt, since))
        .all();
  return rows
    .filter((row) => monthKey(row.startedAt, timezone) === key)
    .reduce((sum, row) => sum + row.spendAmount, 0);
}

export function identitySpendThisMonth(
  db: Database,
  clock: Clock,
  identityId: string,
  timezone: string,
...
export function budgetStatus(
  ...
  return {
    ...
    hasReserveHeadroom:
      identitySpend < policy.identityMonthlyCap + policy.reserve &&

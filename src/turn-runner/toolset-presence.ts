  ...
  return {
    ...
    impl: async (args) => {
      ...
      if (!target) {
        return {
          success: false,
          output: `"${toolArgs.ref ?? ""}" is not a ref — copy the [rN] tag (like r3) from the start of a line you were shown; timestamps and channel ids are labels, not addresses`,
        };
      }
      ...
      if (result.messageId === "undelivered") {
        return {
          success: false,
          output:
            "that didn't send — the surface rejected it after retries. try again, or let it go",
        };
      }
  ...
  return {
    ...
    impl: async (args) => {
      ...
      if (!target) {
        return {
          success: false,
          output: `"${toolArgs.ref ?? ""}" is not a ref — seat the checklist with the [rN] tag of the conversation its work is for`,
        };
      }
      const key = conversationOf(target);
      ...
      pushEffect(ctx, {
        kind: "checklist",
        items: toolArgs.items.length,
        done: toolArgs.items.filter((item) => item.done).length,
      });
      return {
        success: true,
        output: `checklist: ${toolArgs.items.filter((item) => item.done).length}/${toolArgs.items.length} done`,
      };
    },
  };
}
  ...
  return {
    ...
    impl: async (args) => {
      ...
      if (!target)
        return {
          success: false,
          output: "no such ref — step back using an [rN] tag from the conversation you're leaving",
        };
      const key = conversationOf(target);
